...
def main():
    ...
    pdb_id = input("input pdbid:")
    pdb_url = "https://files.rcsb.org/download/{}.pdb".format(pdb_id) 
    eisting = subprocess.getoutput("ls ./pdb_data/") #パス指定

    if pdb_id in eisting:  
        pass
    else:
        subprocess.call("wget {}".format(pdb_url),cwd=r"./pdb_data/",shell=True)  #パス指定
